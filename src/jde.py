...
from .lib.image_utils import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian, xywh2xyxy, xyxy2xywh, generate_anchors, encode_delta
...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __init__(self, opt, root, paths, img_size=(1088, 608), augment=False, transforms=None):
        ...
        for ds, path in paths.items():
            ...
            self.label_files[ds] = [
                x.replace('train', 'labels_with_ids/train').replace('.png', '.txt').replace('.jpg', '.txt')
    ...
    def __getitem__(self, files_index):
        ...
        imgs, labels, img_path, (input_h, input_w) = self.get_data(img_path, label_path)    # LoadImagesAndLabels class function 
                                                                                            # : hsv augment -> resize -> padding -> augment, rotate, affine (for use training)
        ...
        for k in range(num_objs):
            ...
            if h > 0 and w > 0:
                ...
                reg_mask[k] = 1
                ids[k] = label[1]
                #hm: just value 0 -> input dataset # of classes 1 
