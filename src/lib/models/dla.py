...
def get_model_url(data='imagenet', name='dla34', hash='ba72cf86'):
    return os.path.join('http://dl.yf.io/dla/models', data, '{}-{}.pth'.format(name,hash))  # pretrained model
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        ...
        self.relu = nn.ReLU(inplace=True)   # if inplace=True, input overwrite
        self.mish = Mish()
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        ...
        self.bn1 = BatchNorm(bottle_planes, momentum=BN_MOMENTUM)
        ...
        self.bn2 = BatchNorm(bottle_planes, momentum=BN_MOMENTUM)
        self.conv3 = nn.Conv2d(bottle_planes, planes, kernel_size=1, bias=False)
        self.bn3 = BatchNorm(planes, momentum=BN_MOMENTUM)
...
class Root(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, residual):
        ...
        self.bn = BatchNorm(out_channels, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.mish = Mish()
        self.activation = self.mish
        # self.activation = self.relu
...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000, block=BasicBlock, residual_root=False, linear_root=False):
        ...
        self.base_layer = nn.Sequential(
                ...
                BatchNorm(channels[0], momentum=BN_MOMENTUM),
    ...
    def load_pretrained_model(self, data='imagenet', name='dla34', hash='ba72cf86'):
        if name.endswith('.pth'):
            model_weights = torch.load(data + name)     # load path check
        else:
...
class DeformConv(nn.Module):
    def __init__(self, chi, cho):
        ...
        self.actf = nn.Sequential(
                BatchNorm(cho, momentum=BN_MOMENTUM),
                Mish()  # nn.ReLU(inplace=True)
                )
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4, pretrained=True):
    model = DLASeg('dla{}'.format(num_layers), heads, pretrained=pretrained, down_ratio=down_ratio, final_kernel=1, last_level=5, head_conv=head_conv)
    return model
