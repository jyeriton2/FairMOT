...
class VITSeq(nn.Module):
    def __init__(self, base_name, heads, pretrained, image_size, patch_size, head_conv):
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                fc = nn.Sequential(
                        nn.Conv2d(3, head_conv, kernel_size=3, stride=2, padding=1, bias=True),
                        nn.GELU(),
                        nn.Conv2d(head_conv, classes, kernel_size=3, stride=2, padding=1, bias=True))
                if 'hm' in head:
                    fc[-1].bias.data.fill_(-2.19)
                else:
                    fill_fc_weights(fc)
            else:
                fc = nn.Conv2d(3, classes, kernel_size=3, stride=2, padding=1, bias=True)
                if 'hm' in head:
    ...
    def forward(self, x):
        # x = self.base(x)  # ToDo : function definition 
        x = self.vit(x)
...
def get_pose_net(num_layers, heads, head_conv=256, img_size=(512, 912), patch_size=16, pretrained=True):
    if type(img_size) == tuple or type(img_size) == list:
        # cv2 numpy to torch tensor : (b, w, h, c) -> (b, c, h, w)
        assert img_size[1] == 912
        assert img_size[0] == 512
    else:
        if int(num_layers) != 512:
            img_size = num_layers
        assert img_size == 512
